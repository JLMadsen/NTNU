...
plot1.plot(x_train.squeeze(), y_train.squeeze(), 'o', label='$(\\hat x^{(i)},\\hat y^{(i)})$', color='blue')

plot1_f, = plot1.plot([[0], [10]], model.f(np.array([[0], [10]])), color='green', label='$y = f(x) = xW+b$')
...
plot2_loss, = plot2.plot(np.ravel(model.W), np.ravel(model.b), model.loss(x_train, y_train), 'o', color='purple')
